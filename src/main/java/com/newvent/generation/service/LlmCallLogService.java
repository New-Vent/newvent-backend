...
public class LlmCallLogService {
    ...
    static LlmCallLog recordToEntity(RetryService.Trace t, Event event, EventVersion version,
        ...
        String codes = validOk ? null : t.failures().stream()
                .map(f -> f.code())
        ...
        return LlmCallLog.create(event, version, requestId, t.attempt(), modelName, provider,
                ...
                truncated, true, validOk, type, null, codes, createdAt);
    }
     ...
     * version 은 VersionService 확정 전까지 null 허용.
     */
     ...
     * 토큰·응답시간은 null(측정 못 함), 잘림은 false(출력 자체가 없음), fail_codes 는 null.
     * attemptNo 는 호출자가 준다 — 연결 실패가 3차에 나도 그 시도를 변형해 저장한다.
     * version 은 VersionService 확정 전까지 null 허용.
     */
    public LlmCallLog recordCallFailure(Event event, EventVersion version, UUID requestId,
            String modelName, String provider, FailureType type, int attemptNo) {
        LlmCallLog row = LlmCallLog.create(event, version, requestId, attemptNo, modelName, provider,
                null, null, null, false, false, false, type, null, null, clock.instant());
